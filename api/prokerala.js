export default async function handler(req, res) {
  ...
  // 2. Get western natal chart
  const datetime = `${birthDate}T${birthTime}:00+07:00`;
  const astroRes = await fetch(
    `https://api.prokerala.com/v2/astrology/western/natal-chart?coordinates=${lat},${lon}&datetime=${encodeURIComponent(datetime)}&house_system=placidus`,
